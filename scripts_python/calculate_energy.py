...
def estimate_bin_F_pair(name, factor_bcell, k_bin, n1, n2, vol_tot, ax, gamma, gen_curves_flag, k_reflex):
    F_norm = []
    ...
    #mask = (x_bin >= np.min(aL_bin)) & (x_bin <= np.max(aL_bin))
...
def estimate_part_F_pair(part, part_cell, factor_aL_part, ni, k_part, vol_tot, gen_curves_flag, k_reflex_part, gamma):
    ...
    packing_list = np.linspace(0.1,1.0,100)
    # Evaluación del polinomio ajustado
    if gamma<0.52 and part == 'part2':
        x_cell = np.power(vol_tot/packing_list, 1./3.)
        mask = (x_cell > min(aL_cell)) & (x_cell < max(aL_cell))
        x_cell = x_cell[mask]
