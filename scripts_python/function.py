...
def extract_params_init(params_init, cond):
    ...
    while i < len(lines):
        ...
        if line == "!name":
            ...
            data["gamma list"] = [np.round(float(x),3) for x in lines[i+1].strip("[]\n").split(",")]
            i += 1
...
def update_T(DEF, T):
    ...
    T01 = [313.15,0.6750,0.00970,0.3729,640.742]
    T02 = [333.15,0.6260,0.0094,0.7646,621.596]
    ...
    T07 = [452.15,0.4220,0.00785,12.776,477.51]
